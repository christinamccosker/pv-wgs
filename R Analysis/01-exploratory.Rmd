...
meta <- 
  ...
  filter(!sample.no == "43" & 
         ...
         !sample.no == "79" & 
         !sample.no == "106") %>% 
  mutate(classification2 = ifelse(classification == "Non-survivor", "Case", "Control"))
...
tufts_outbreak <-
  meta %>% 
  select(classification2, pdv.tufts) %>% 
  filter(!pdv.tufts == "N/A") %>% 
  mutate(pdv.tufts=factor(pdv.tufts, levels=c("Positive", "Negative", "N/A"))) %>% 
  ggplot(data=.) +
  geom_mosaic(aes(x=product(pdv.tufts,classification2), fill=pdv.tufts)) +
...
map <-
  ...
  labs(x=NULL, y=NULL) +
  annotate(geom = "text", x = -68.5, y = 42.5, label = "Atlantic Ocean", 
           fontface = "italic", color = "black", size = 3.5) +
  annotate(geom = "text", x = -70, y = 44.5, label = "Maine", 
           fontface = "italic", color = "black", size = 3.5) +
  annotate(geom = "text", x = -71.5, y = 43.5, label = "New \nHampshire", 
           fontface = "italic", color = "black", size = 3.5) +
  annotate(geom = "text", x = -71.5, y = 42.3, label = "Massachusetts", 
           fontface = "italic", color = "black", size = 3.5) +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        text = element_text(size=12),
        legend.position = "inside",
        legend.position.inside = c(0.83,0.095)) 
map
