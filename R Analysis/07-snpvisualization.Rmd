...
plot2 <- grid.arrange(arrangeGrob(plot1, snp.frequency.nervous.plot,
                                  ncol = 2),
                      bottom = textGrob("Frequency", gp = gpar(fontsize = 13)), 
                      left = textGrob("Gene", rot = 90, gp = gpar(fontsize = 13)))
...
snp.fst.frequency <- 
  ...
  mutate(genotype = ifelse(genotype == "0", "AA", 
                           ifelse(genotype == "1", "AB", "BB")),
         classification = ifelse(classification == "Non-survivor", "Case", "Control"))
